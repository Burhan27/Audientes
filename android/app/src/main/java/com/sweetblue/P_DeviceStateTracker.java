...
final class P_DeviceStateTracker extends PA_StateTracker
{
	...
	@Override protected void onStateChange(final int oldStateBits, final int newStateBits, final int intentMask, final int gattStatus)
	{
		...
		{
			final BleDevice.StateListener.StateEvent event = new BleDevice.StateListener.StateEvent(m_device, oldStateBits, newStateBits, intentMask, gattStatus);
		...
		final BleDeviceConfig.BondFilter.StateChangeEvent bondStateChangeEvent = new BleDeviceConfig.BondFilter.StateChangeEvent(m_device, oldStateBits, newStateBits, intentMask, gattStatus);
		
		final BleDeviceConfig.BondFilter.Please please = bondFilter.onEvent(bondStateChangeEvent);
		
		m_device.getManager().getLogger().checkPlease(please, BleDeviceConfig.BondFilter.Please.class);
