...
final class P_PollManager
{
	...
	private static class PollingReadListener extends P_WrappingReadWriteListener
	{
		...
		PollingReadListener(BleDevice.ReadWriteListener readWriteListener, P_SweetHandler handler, boolean postToMain)
		{
		...
		private boolean hasListener(BleDevice.ReadWriteListener listener)
		{
			return listener == m_overrideListener;
		}
		
		private void addListener(BleDevice.ReadWriteListener listener)
		{
	...
	private static class TrackingWrappingReadListener extends PollingReadListener
	{
		...
		TrackingWrappingReadListener(BleDevice.ReadWriteListener readWriteListener, P_SweetHandler handler, boolean postToMain)
		{
	...
	private static class CallbackEntry
	{
		...
		public CallbackEntry(BleDevice device, final UUID serviceUuid, UUID charUuid, DescriptorFilter descriptorFilter, double interval, BleDevice.ReadWriteListener readWriteListener, boolean trackChanges, boolean usingNotify)
		{
		...
		boolean isFor(final UUID serviceUuid, final UUID charUuid, DescriptorFilter descriptorFilter, Double interval_nullable, BleDevice.ReadWriteListener readWriteListener_nullable, boolean usingNotify)
		{
			return
		...
		void onCharacteristicChangedFromNativeNotify(byte[] value)
		{
			...
			BleDevice.ReadWriteListener.Type type = P_DeviceServiceManager.modifyResultType(characteristic, BleDevice.ReadWriteListener.Type.NOTIFICATION);
			int gattStatus = BleStatuses.GATT_STATUS_NOT_APPLICABLE;
			
			if( value == null )
			{
				BleDevice.ReadWriteListener.ReadWriteEvent result = new BleDevice.ReadWriteListener.ReadWriteEvent(m_device, m_serviceUuid, m_charUuid, null, m_descriptorFilter, type, BleDevice.ReadWriteListener.Target.CHARACTERISTIC, value, BleDevice.ReadWriteListener.Status.NULL_DATA, gattStatus, 0.0, 0.0, /*solicited=*/true);
			...
			else
			{
				if( value.length == 0 )
				{
					BleDevice.ReadWriteListener.ReadWriteEvent result = new BleDevice.ReadWriteListener.ReadWriteEvent(m_device, m_serviceUuid, m_charUuid, null, m_descriptorFilter, type, BleDevice.ReadWriteListener.Target.CHARACTERISTIC, value, BleDevice.ReadWriteListener.Status.EMPTY_DATA, gattStatus, 0.0, 0.0, /*solicited=*/true);
					m_device.invokeReadWriteCallback(m_pollingReadListener, result);
				}
				else
				{
					BleDevice.ReadWriteListener.ReadWriteEvent result = new BleDevice.ReadWriteListener.ReadWriteEvent(m_device, m_serviceUuid, m_charUuid, null, m_descriptorFilter, type, BleDevice.ReadWriteListener.Target.CHARACTERISTIC, value, BleDevice.ReadWriteListener.Status.SUCCESS, gattStatus, 0.0, 0.0, /*solicited=*/true);
					m_device.invokeReadWriteCallback(m_pollingReadListener, result);
				}
			}
			...
			if( m_timeTracker >= m_interval )
			{
				...
				{					
					if( !m_waitingForResponse )
					{
						...
						BleDevice.ReadWriteListener.Type type = trackingChanges() ? BleDevice.ReadWriteListener.Type.PSUEDO_NOTIFICATION : BleDevice.ReadWriteListener.Type.POLL;
	...
	void startPoll(final UUID serviceUuid, final UUID charUuid, final DescriptorFilter decriptorFilter, double interval, BleDevice.ReadWriteListener listener, boolean trackChanges, boolean usingNotify)
	{
	...
	void stopPoll(final UUID serviceUuid, final UUID characteristicUuid, DescriptorFilter descriptorFilter, Double interval_nullable, BleDevice.ReadWriteListener listener, boolean usingNotify)
	{
		if( m_device.isNull() )  return;
	...
	void enableNotifications_assumesWeAreConnected()
	{
		for( int i = 0; i < m_entries.size(); i++ )
		{
			...
			if( ithEntry.usingNotify() )
			{
				...
				{
					BleDevice.ReadWriteListener.ReadWriteEvent earlyOutResult = m_device.serviceMngr_device().getEarlyOutEvent(ithEntry.m_serviceUuid, ithEntry.m_charUuid, Uuids.INVALID, ithEntry.m_descriptorFilter, P_Const.EMPTY_FUTURE_DATA, BleDevice.ReadWriteListener.Type.ENABLING_NOTIFICATION, BleDevice.ReadWriteListener.Target.CHARACTERISTIC);
				...
				{
					BleDevice.ReadWriteListener.ReadWriteEvent result = newAlreadyEnabledEvent(characteristic, ithEntry.m_serviceUuid, ithEntry.m_charUuid, ithEntry.m_descriptorFilter);
	...
	BleDevice.ReadWriteListener.ReadWriteEvent newAlreadyEnabledEvent(BluetoothGattCharacteristic characteristic, final UUID serviceUuid, final UUID characteristicUuid, final DescriptorFilter descriptorFilter)
	{
		...
		BleDevice.ReadWriteListener.ReadWriteEvent result = new BleDevice.ReadWriteListener.ReadWriteEvent(m_device, serviceUuid, characteristicUuid, Uuids.CLIENT_CHARACTERISTIC_CONFIGURATION_DESCRIPTOR_UUID, descriptorFilter, BleDevice.ReadWriteListener.Type.ENABLING_NOTIFICATION, BleDevice.ReadWriteListener.Target.DESCRIPTOR, writeValue, BleDevice.ReadWriteListener.Status.SUCCESS, gattStatus, 0.0, 0.0, /*solicited=*/true);
