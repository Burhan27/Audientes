...
//			m_device.onDisconnecting();
    ...
    private void onNativeConnectFail(final BluetoothGatt gatt, final int gattStatus)
    {
        ...
        {
            m_device.onNativeConnectFail((PE_TaskState) null, gattStatus, BleNode.ConnectionFailListener.AutoConnectUsage.UNKNOWN);
        }
    }
...
//		}
    ...
    public final void onNativeBondStateChanged_updateThread(int previousState, int newState, int failReason)
    {
...

            {
                ...
                if (m_device.m_nativeWrapper.isNativelyUnbonded())
                {
                    if (!m_queue.succeed(P_Task_Unbond.class, m_device))
                    {
                        ...
                        {
                            m_device.m_bondMngr.onNativeBondFailed(PA_StateTracker.E_Intent.UNINTENTIONAL, BleDevice.BondListener.Status.FAILED_EVENTUALLY, failReason, false);
                        }
                        else
                        {
                            m_device.m_bondMngr.onNativeUnbond(PA_StateTracker.E_Intent.UNINTENTIONAL);
                        }
                    }
                }
                else
        ...
        else if (newState == BluetoothDevice.BOND_BONDED)
        {
            ...
            {
                m_device.m_bondMngr.onNativeBond(PA_StateTracker.E_Intent.UNINTENTIONAL);
            }
        }
    }
