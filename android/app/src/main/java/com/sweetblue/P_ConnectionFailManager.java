...
final class P_ConnectionFailManager
{
	...
	private final ArrayList<BleDevice.ConnectionFailListener.ConnectionFailEvent> m_history = new ArrayList<BleDevice.ConnectionFailListener.ConnectionFailEvent>();
	...
	int/*__PE_Please*/ onConnectionFailed(BleDevice.ConnectionFailListener.Status reason_nullable, BleDevice.ConnectionFailListener.Timing timing, boolean isAttemptingReconnect_longTerm, int gattStatus, int bondFailReason, BleDeviceState highestStateReached, BleNode.ConnectionFailListener.AutoConnectUsage autoConnectUsage, BleDevice.ReadWriteListener.ReadWriteEvent txnFailReason)
	{
		if( reason_nullable == null )  return BleDevice.ConnectionFailListener.Please.PE_Please_DO_NOT_RETRY;
		...
		final BleDevice.ConnectionFailListener.ConnectionFailEvent moreInfo = new BleDevice.ConnectionFailListener.ConnectionFailEvent
		(
		...
		int retryChoice__PE_Please = m_device.is(BleDeviceState.RECONNECTING_SHORT_TERM) ? BleNode.ConnectionFailListener.Please.PE_Please_DO_NOT_RETRY : invokeCallback(moreInfo);
		
		//--- DRK > Retry choice doesn't matter if we're attempting reconnect.
		retryChoice__PE_Please = !isAttemptingReconnect_longTerm ? retryChoice__PE_Please : BleNode.ConnectionFailListener.Please.PE_Please_DO_NOT_RETRY;
...
//		retryChoice__PE_Please = m_device.is_internal(BleDeviceState.CONNECTING_OVERALL) ? Please.PE_Please_DO_NOT_RETRY : retryChoice__PE_Please;
		...
		{
			retryChoice__PE_Please = BleNode.ConnectionFailListener.Please.PE_Please_DO_NOT_RETRY;
		}
		else
		{
			...
			if( wasRunning && !reconnectMngr.isRunning() )
			{
				if( m_device.is(RECONNECTING_LONG_TERM) )
				{
					...
					m_device.stateTracker_main().update(PA_StateTracker.E_Intent.UNINTENTIONAL, gattStatus, RECONNECTING_LONG_TERM, false);
					m_triedReconnectingLongTerm = true;
				}
		...
		if( !m_triedReconnectingLongTerm && retryChoice__PE_Please != BleNode.ConnectionFailListener.Please.PE_Please_NULL && BleNode.ConnectionFailListener.Please.isRetry(retryChoice__PE_Please) && !m_device.is(CONNECTED))
		{
			if (m_device.isAny_internal(CONNECTED, CONNECTING, CONNECTING_OVERALL))
			{
				m_device.setStateToDisconnected(isAttemptingReconnect_longTerm, true, PA_StateTracker.E_Intent.UNINTENTIONAL, gattStatus, /*forceMainStateTracker=*/false, P_BondManager.OVERRIDE_EMPTY_STATES);
			}
		...
		else
		{
			...
			if (m_device.isAny_internal(CONNECTED, CONNECTING, CONNECTING_OVERALL))
			{
				boolean isretryConnectOverall = BleNode.ConnectionFailListener.Please.isRetry(retryChoice__PE_Please) && retryConnectOverall;
				m_device.setStateToDisconnected(isAttemptingReconnect_longTerm, isretryConnectOverall, PA_StateTracker.E_Intent.UNINTENTIONAL, gattStatus, /*forceMainStateTracker=*/false, P_BondManager.OVERRIDE_EMPTY_STATES);
			}
		}
	...
	ArrayList<BleDevice.ConnectionFailListener.ConnectionFailEvent> getHistory()
	{
		return new ArrayList<>(m_history);
	}

	private void addToHistory(BleDevice.ConnectionFailListener.ConnectionFailEvent event)
	{
	...
	int/*__PE_Please*/ invokeCallback(final BleDevice.ConnectionFailListener.ConnectionFailEvent moreInfo)
	{
		int retryChoice__PE_Please = BleNode.ConnectionFailListener.Please.PE_Please_NULL;
		
		if( m_connectionFailListener != null )
		{
			final BleNode.ConnectionFailListener.Please please = m_connectionFailListener.onEvent(moreInfo);
			retryChoice__PE_Please = please != null ? please.please() : BleNode.ConnectionFailListener.Please.PE_Please_NULL;
			
			m_device.getManager().getLogger().checkPlease(please, BleNode.ConnectionFailListener.Please.class);
		}
		else if( m_device.getManager().m_defaultConnectionFailListener != null )
		{
			final BleNode.ConnectionFailListener.Please please = m_device.getManager().m_defaultConnectionFailListener.onEvent(moreInfo);
			retryChoice__PE_Please = please != null ? please.please() : BleNode.ConnectionFailListener.Please.PE_Please_NULL;
		...
		{
			final BleNode.ConnectionFailListener.Please please = BleDevice.DEFAULT_CONNECTION_FAIL_LISTENER.onEvent(moreInfo);
			retryChoice__PE_Please = please != null ? please.please() : BleNode.ConnectionFailListener.Please.PE_Please_NULL;

			m_device.getManager().getLogger().checkPlease(please, BleNode.ConnectionFailListener.Please.class);
		}

		retryChoice__PE_Please = retryChoice__PE_Please != BleNode.ConnectionFailListener.Please.PE_Please_NULL ? retryChoice__PE_Please : BleNode.ConnectionFailListener.Please.PE_Please_DO_NOT_RETRY;
		
		return retryChoice__PE_Please;
	}
	
	public void setListener(BleDevice.ConnectionFailListener listener)
	{
