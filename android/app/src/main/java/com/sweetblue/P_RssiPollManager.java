...
final class P_RssiPollManager
{
	private static class CustomListener extends P_WrappingReadWriteListener
	{
		...
		CustomListener(P_RssiPollManager thisMngr, BleDevice.ReadWriteListener listener, P_SweetHandler handler, boolean postToMain)
		{
	...
	final void start(double interval, BleDevice.ReadWriteListener listener_nullable)
	{
		if( interval > 0.0 )
		{
		...
		if( m_timeTracker != DISABLE_TIMER )
		{
			...
			if( m_timeTracker >= m_interval && !m_waitingOnResponse )
			{
				if( m_device.is(BleDeviceState.INITIALIZED) )
				{
					...
					m_device.readRssi_internal(BleDevice.ReadWriteListener.Type.POLL, m_listener);
				}
				else if (m_cachedRssi != Integer.MAX_VALUE)
				{
					final BleDevice.ReadWriteListener.ReadWriteEvent event = new BleDevice.ReadWriteListener.ReadWriteEvent(m_device, BleDevice.ReadWriteListener.Type.POLL, m_cachedRssi, BleDevice.ReadWriteListener.Status.SUCCESS, BleStatuses.GATT_STATUS_NOT_APPLICABLE, 0.0, 0.0, false);
