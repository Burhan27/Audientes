...
final class P_TransactionManager
{
	...
	{
		@Override public void onTransactionEnd(BleTransaction txn, EndReason reason, BleDevice.ReadWriteListener.ReadWriteEvent txnFailReason)
		{
			clearQueueLock();
			...
			if (txn == m_authTxn )
			{
				...
				{
					m_device.disconnectWithReason(BleDevice.ConnectionFailListener.Status.AUTHENTICATION_FAILED, BleDevice.ConnectionFailListener.Timing.NOT_APPLICABLE, BleStatuses.GATT_STATUS_NOT_APPLICABLE, BleStatuses.BOND_FAIL_REASON_NOT_APPLICABLE, txnFailReason);
				}
			}
			else if (txn == m_initTxn )
			{
				...
				{
					m_device.disconnectWithReason(BleDevice.ConnectionFailListener.Status.INITIALIZATION_FAILED, BleDevice.ConnectionFailListener.Timing.NOT_APPLICABLE, BleStatuses.GATT_STATUS_NOT_APPLICABLE, BleStatuses.BOND_FAIL_REASON_NOT_APPLICABLE, txnFailReason);
				}
			}
...
//	}
	...
	void onReadWriteResult(BleDevice.ReadWriteListener.ReadWriteEvent result)
	{
		resetReadWriteResult();
...
//			m_device.getManager().ASSERT(m_otaTxn == null);
		...
		m_device.stateTracker_main().append(PERFORMING_OTA, PA_StateTracker.E_Intent.INTENTIONAL, BleStatuses.GATT_STATUS_NOT_APPLICABLE);
