...
//			m_device.getManager().ASSERT(!m_device.is(E_DeviceState.ATTEMPTING_RECONNECT));
	...
	{
		final BleNodeConfig.ReconnectFilter filter = m_device.conf_device().reconnectFilter;

		return filter != null ? filter : m_device.conf_mngr().reconnectFilter;
	}
	
	private double getNextTime(BleDevice.ConnectionFailListener.ConnectionFailEvent connectionFailInfo)
	{
		final BleNodeConfig.ReconnectFilter filter = getFilter();
		...
		{
			final BleNodeConfig.ReconnectFilter.Type type = m_isShortTerm ? BleNodeConfig.ReconnectFilter.Type.SHORT_TERM__SHOULD_TRY_AGAIN : BleNodeConfig.ReconnectFilter.Type.LONG_TERM__SHOULD_TRY_AGAIN;
			...
			final BleNodeConfig.ReconnectFilter.Please please = filter.onEvent(EVENT);
			
			m_device.getManager().getLogger().checkPlease(please, BleNodeConfig.ReconnectFilter.Please.class);
	...
	void onConnectionFailed(final BleDevice.ConnectionFailListener.ConnectionFailEvent connectionFailInfo)
	{
		if( !isRunning() )
		{
			return;
	...
	{
		BleNodeConfig.ReconnectFilter persistFilter = getFilter();
		
		if( persistFilter == null )  return;

		final BleNodeConfig.ReconnectFilter.Type type = m_isShortTerm ? BleNodeConfig.ReconnectFilter.Type.SHORT_TERM__SHOULD_CONTINUE : BleNodeConfig.ReconnectFilter.Type.LONG_TERM__SHOULD_CONTINUE;
		...
		final BleNodeConfig.ReconnectFilter.Please please = persistFilter.onEvent(EVENT);
		...
		if( please == null || false == please.shouldPersist() )
		{
			...
			{
				m_device.stateTracker_main().update(PA_StateTracker.E_Intent.UNINTENTIONAL, BleStatuses.GATT_STATUS_NOT_APPLICABLE, RECONNECTING_LONG_TERM, false);
