...
/**
 * A simple interface whose implementations should be passed to methods like {@link BleDevice#write(UUID, FutureData)}
 ...
 * the time through {@link BleDevice#write(UUID, byte[])} then the operation might spend a second or two in SweetBlue's internal job
...
public interface FutureData
{
	...
	/**
	 * Return the data that should (for example) be sent to a peripheral through {@link BleDevice#write(UUID, FutureData)}.
	 */
	byte[] getData();
}
