...
final class P_BondManager
{
		...
		if( task.getClass() == P_Task_Bond.class )
		{
			...
			if( state.isEndingState() )
			{
				...
				else
				{
					...
					final BleDevice.BondListener.Status status;
					
					if( state == PE_TaskState.TIMED_OUT )
					{
						status = BleDevice.BondListener.Status.TIMED_OUT;
					}
					else if( state == PE_TaskState.FAILED_IMMEDIATELY )
					{
						status = BleDevice.BondListener.Status.FAILED_IMMEDIATELY;
					}
					else
					{
						status = BleDevice.BondListener.Status.FAILED_EVENTUALLY;
					}
	...
	final void onNativeBond(final PA_StateTracker.E_Intent intent)
	{
		...
		if( !wasAlreadyBonded )
		{
			invokeCallback(BleDevice.BondListener.Status.SUCCESS, BleStatuses.BOND_FAIL_REASON_NOT_APPLICABLE, intent.convert());
		}
	}
	
	private boolean failConnection(final BleDevice.BondListener.Status status)
	{
	...
	final Object[] getOverrideBondStatesForDisconnect(BleDevice.ConnectionFailListener.Status connectionFailReasonIfConnecting)
	{
		final Object[] overrideBondingStates;
		
		if( connectionFailReasonIfConnecting == BleDevice.ConnectionFailListener.Status.BONDING_FAILED )
		{
	...
	final void onNativeBondFailed(final PA_StateTracker.E_Intent intent, final BleDevice.BondListener.Status status, final int failReason, final boolean wasDirect)
	{
	...
	final BleDevice.BondListener.BondEvent invokeCallback(BleDevice.BondListener.Status status, int failReason, State.ChangeIntent intent)
	{
		final BleDevice.BondListener.BondEvent event = new BleDevice.BondListener.BondEvent(m_device, status, failReason, intent);
	...
	final void invokeCallback(final BleDevice.BondListener.BondEvent event)
	{		
		if( m_listener != null )
		{
		...
		final BleDevice.BondListener listener = m_device.getManager().m_defaultBondListener;
