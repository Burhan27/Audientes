...
import com.sweetblue.annotations.Extendable;
...
public class BleDeviceConfig extends BleNodeConfig implements Cloneable
{
	...
	/**
	 * The default value used for {@link BondRetryFilter.DefaultBondRetryFilter}. Bond retries only apply when calling {@link BleDevice#bond()}, or {@link BleDevice#bond(BleDevice.BondListener)}.
	 ...
	 * {@link SharedPreferences}. If true, this means calls to {@link BleDevice#getName_override()} will return the same thing
	 ...
	 * in {@link BleDevice} upon device reconnection. Basically, if you enable notifications in an {@link BleTransaction.Init} transaction,
	...
	/**
	 * Default is <code>true</code> - whether to automatically renegotiate the MTU size that was set via {@link BleDevice#setMtu(int, BleDevice.ReadWriteListener)}, or
	 * {@link BleDevice#setMtu(int)}. If you use either of those methods in a {@link BleTransaction.Init} transaction, you should set
	 * this to <code>false</code>, as the transaction will run on reconnection.
	 */
	public boolean autoNegotiateMtuOnReconnect					= true;

	/**
	 * Default is <code>null</code> - This callback is used after calling {@link BleDevice#setMtu(int, BleDevice.ReadWriteListener)} or {@link BleDevice#setMtu(int)}, if the
	 ...
	 * inducing {@link BleDevice.ConnectionFailListener#onEvent(BleDevice.ConnectionFailListener.ConnectionFailEvent)}
	 ...
	 * BLE. This means that if discovering services, or {@link BleTransaction.Init}, or {@link BleTransaction.Auth}
	 ...
	 * after it's {@link BleDeviceState#CONNECTED}. You may also use {@link BleDevice#startRssiPoll(Interval, BleDevice.ReadWriteListener)} for more control and feedback.
	 */
	@Nullable(Prevalence.NORMAL)
	 ...
	 * experimentally, simply run a sample app and use {@link BleDevice#startRssiPoll(Interval, BleDevice.ReadWriteListener)} and spit {@link BleDevice#getRssi()}
	...
	/**
	 * Set a default {@link BleTransaction.Auth} which will be used when
	...
	/**
	 * Set a default {@link BleTransaction.Auth} factory which will be used to dispatch a new instance
	...
	/**
	 * Set a default {@link BleTransaction.Init} which will be used when
	 * connecting to a {@link BleDevice}. This transaction will also be called if the {@link BleDevice} has
	 * to reconnect for any reason.
	 *
	 * @deprecated This is still here only so we don't break current builds. It will be removed in version 3. Use
	...
	/**
	 * Set a default {@link BleTransaction.Init} factory which will be used to dispatch a new instance
	 * of the transaction when connecting to a {@link BleDevice}. This transaction will also be called if the {@link BleDevice} has
	 * to reconnect for any reason.
	 */
	@Nullable(Prevalence.NORMAL)
	 ...
	 * of {@link BleNode.ConnectionFailListener.ConnectionFailEvent}s. Note that this will always be
	...
	public static interface BondFilter
	{
		...
		{
			/**
			 * Started from {@link BleDevice#read(UUID, BleDevice.ReadWriteListener)}, {@link BleDevice#startPoll(UUID, Interval, BleDevice.ReadWriteListener)}, etc.
			 */
			READ,

			/**
			 * Started from {@link BleDevice#write(UUID, byte[], BleDevice.ReadWriteListener)} or overloads.
			 */
			WRITE,

			/**
			 * Started from {@link BleDevice#enableNotify(UUID, BleDevice.ReadWriteListener)} or overloads.
			 */
			ENABLE_NOTIFY;
		}
		...
		public static class Please
		{
			...
			private final BleDevice.BondListener m_bondListener;

			Please(Boolean bond, BleDevice.BondListener listener)
			{
				m_bond = bond;
			...
			BleDevice.BondListener listener()
			{
			...
			/**
			 * Same as {@link #bondIf(boolean)} but lets you pass a {@link BleDevice.BondListener} as well.
			 */
			public static Please bondIf(boolean condition, BleDevice.BondListener listener)
			{
				return condition ? bond(listener) : doNothing();
			}

			/**
			 * Same as {@link #bond()} but lets you pass a {@link BleDevice.BondListener} as well.
			 */
			public static Please bond(BleDevice.BondListener listener)
			{
