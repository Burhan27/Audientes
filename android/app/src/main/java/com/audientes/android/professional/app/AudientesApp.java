...
import com.audientes.android.professional.device.sim.WaveSimScanner;
...
import dk.sens.sweetblue.SweetBlueBLEScannerNordic;
...
*/
    ...
    public void onDeviceVersionReceived(byte major, byte minor, byte patch)
    {
        if (getCurrentActivity() != null)
        {
           // getCurrentActivity().showMessage("Device Version", String.format("v%d.%d.%d", major, minor, patch));
        }
    }

    public void onDeviceTimeReceived(long tx)
    {
        if (getCurrentActivity() != null)
        {
            //getCurrentActivity().showMessage("Device Time", String.format("%d", tx));
        }
    }

    public void onTestResult(TestResult result)
    {
        mHistory.addResult(result);
        if (getCurrentActivity() != null)
        {
            //getCurrentActivity().onDeviceTestResult(result);
        }
    }
}
