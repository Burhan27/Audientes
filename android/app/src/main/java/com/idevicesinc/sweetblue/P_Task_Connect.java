...
final class P_Task_Connect extends PA_Task_RequiresBleOn
{
	...
	public P_Task_Connect(BleDevice device, PA_Task.I_StateListener listener)
	{
		this(device, listener, true, null);
	}
	
	public P_Task_Connect(BleDevice device, PA_Task.I_StateListener listener, boolean explicit, PE_TaskPriority priority)
	{
...
//			this.fail();
	...
	@Override
    public boolean isCancellableBy(final PA_Task task)
	{
	...
	@Override
    protected boolean isSoftlyCancellableBy(final PA_Task task)
	{
