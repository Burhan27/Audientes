...
final class P_ServerServiceManager extends PA_ServiceManager
{
	...
	@Override
    public final BleServiceWrapper getServiceDirectlyFromNativeNode(final UUID uuid)
	{
	...
	private boolean alreadyAddingOrAdded(final BluetoothGattService serviceToBeAdded)
	{
		...
		else
		{
			...
			{
				@Override
                public Please next(P_Task_AddService next)
				{
	...
	public final BluetoothGattService remove(final UUID serviceUuid)
	{
		...
		if( service.isNull() )
		{
			...
			{
				@Override
                public Please next(final P_Task_AddService next)
				{
