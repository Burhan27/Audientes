...
public class Backend_HistoricalDataList_Default implements Backend_HistoricalDataList
{
	...
	@Override
    public void init(final Backend_HistoricalDatabase database, final UpdateLoop updateLoop, final String macAddress, final UUID uuid, final String uuidName, final boolean hasExistingTable)
	{
		m_database = database;
	...
	@Override
    public void add_single(HistoricalData historicalData, final int persistenceLevel, long limit)
	{
	...
	@Override
    public void add_multiple(Iterator<HistoricalData> historicalData, final int persistenceLevel, final long limit)
	{
	...
	@Override
    public void add_multiple(ForEach_Returning<HistoricalData> historicalData, final int persistenceLevel, final long limit)
	{
		int i = 0;
	...
	@Override
    public Iterator<HistoricalData> getIterator(EpochTimeRange range)
	{
		if( isDataInRange(range) )
		{
	...
	@Override
    public boolean doForEach(EpochTimeRange range, Object forEach)
	{
		if( isDataInRange(range) )
		{
	...
	@Override
    public void load(AsyncLoadCallback callback_nullable)
	{
		printWarning_write();
	}
